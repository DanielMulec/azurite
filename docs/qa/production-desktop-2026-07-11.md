...
| P2       | Browser Back can restore URL and content while leaving the wrong sidebar item selected. | Product Vision content returned while Technical Architecture remained `aria-current`.  | Classify against the pre-7B baseline before the final 7B synthetic phone re-run; repair in 7B if regressed there, otherwise create a separate route-state correctness slice. |
